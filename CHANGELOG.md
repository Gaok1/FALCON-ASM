...
### Compatibilidade RISC-V

- **`fence`** — instrução base RV32I implementada (encode/decode/exec como nop em single-core); programas gerados por GCC que emitem `fence` não travam mais
- **`li` full 32-bit** — `li rd, imm` agora aceita qualquer valor i32; expande automaticamente para `lui + addi` quando fora do range −2048..2047 (ex: `li a0, 0x10000` funciona)
- **Branch pseudos com dois registradores** — `bgt`, `ble`, `bgtu`, `bleu` (permutam operandos para `blt`/`bge`/`bltu`/`bgeu`)
- **Branch pseudos vs zero** — `bltz`, `bgez`, `blez`, `bgtz` (comparam com `x0`)
- **Set pseudos** — `seqz rd, rs` (`sltiu 1`), `snez` (`sltu x0`), `sltz` (`slt x0`), `sgtz` (`slt x0`)
- **Pseudos aritméticos** — `neg rd, rs` (`sub x0`), `not rd, rs` (`xori −1`)
- **Ghost spans no editor** — todas as novas instruções têm autocomplete/hint de operandos na IDE

### Documentação

- **`claudeProblems/riscv_roadmap.md`** — documento detalhado das próximas lacunas de compatibilidade RISC-V: `.word label`, Zicsr/rdcycle, RV32F, A extension, M-mode privilege
